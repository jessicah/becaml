...
external b_polygon_polygon : #be_interne -> pointer array -> int32 -> pointer = "b_polygon_polygon"
...
class be_polygon =
	...
	fun ~pointList ~numPoints ->
		self#set_interne(b_polygon_polygon self (Array.map (fun p -> p#get_interne()) pointList) numPoints);
