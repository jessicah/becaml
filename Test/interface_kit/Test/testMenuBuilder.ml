...
class testMenuBuilder =
	...
	method buildTestMenu ~size =
		...
		try 
			...
			in
			!pTypicalItem#getBitmapSize ~width:itemWidth ~height:itemHeight;
		...
		method loadTestBitmap ~i ~j ~size =
			...
			try
				...
				then begin
						Printf.printf "chargement de LARGE_ICON\n";flush stdout;
						...
						Printf.printf "chargement de MINI_ICON\n";flush stdout;
				...
				in
				ignore (Posix.String.memcpy ~dst:destBits ~src:(int8_array_to_be !bits) ~len:(Int32.of_int !byteLen));
				pBitmap;
